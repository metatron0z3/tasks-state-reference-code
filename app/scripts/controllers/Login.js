...
function LoginCtrl(
  ...
  function legacyLogin(form) {
    ...
    TroopApi.login({
      loginId: phoneUtils.formatE164($localStorage.newAccount.phone),
  ...
  function displayGoogleOauth() {
    ...
    .then(function apiLoginWithOauth(userToken) {
      ...
      //console.log(  $.extend({},Me.firebaseUser) );
    ...
    .then(function apiLoginWithOauthSuccess(resp) {
      ...
      TroopLogger.debug(logConfig, 'displayGoogleOauth', 'apiLoginWithOauthSuccess', resp);
  ...
  function createAccount() {
    ...
    .then(function tryToLoadTroopMember() {
      ...
      TroopLogger.debug(logConfig, 'tryToLoadTroopMember');
      return Me.loadTroopMember(vm.troopId, Me.trooper.troops[HELP_TROOP_ID].memberId);
    })
    .then(function waitForTroopMemberToLoad() {
    ...
    .then(function tryToLoadBoard() {
      ...
      .then(function loadBoardResolve(boardId) {
        //vm.troopId = HELP_TROOP_ID;
        if ( HELP_TROOP_BOARD_ID && HELP_TROOP_BOARD_ID !== '' ) {
          vm.starterBoardId = HELP_TROOP_BOARD_ID;
        }
        else {
  ...
  function accountExistsWithDifferentCredential(pendingCredential, email) {
  ...
  function redirectOauth(provider) {
    ...
    TroopLogger.debug(logConfig, 'redirectOauth');
