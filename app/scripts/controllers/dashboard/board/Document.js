(function() {
  ...
  /**
   * @ngdoc function
   * @name webClientApp.controller:DocumentCtrl
   * @description
   * # DocumentCtrl
  ...
  function DocumentCtrl(
    ...
    var logConfig = {
      slug: 'controller: DocumentCtrl - ',
      path: [ 'controllers', 'dashboard', 'board', 'Document.js']
    };
    ...
    this.orderNotes = function(card) {
      ...
      $scope.notes.orderedList = _.sortBy(that.notes.list, function(note, index) {
    ...
    $scope.newNoteFocus = function() {

      if ( Me.troopMember.troopPermission === 'guest' ) {
        $scope.showPublicTroopJoinModal();
      }
    }
    $scope.newNoteTyping = function() {
    ...
    $rootScope.$broadcast('document-view-loaded');
    ...
    $scope.$on('$destroy', function() {
      ...
      if ( that.notes.list.hasOwnProperty('$destroy') ) {

        that.notes.list.$destroy();
      }
    ...
    .then(function complete() {
      ...
      $scope.loading = false;
      TroopLogger.debug(logConfig, 'documentView all loaded - Me.currentBoard',$.extend({},Me.currentBoard));
    })
    .catch(function(error) {
      console.log(error)
    });
